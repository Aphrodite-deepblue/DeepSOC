# DeepSOC——基于多智能体协同与 RAG 架构的智能安全运营系统

DeepSOC 是面向安全日志分析与情报检索场景的 SOC 原型系统，采用前后端分离架构：

- 后端：Django + django-ninja + ChromaDB + LlamaIndex + OpenAI-compatible LLM 路由
- 前端：Vue 3 + Pinia + Naive UI + ECharts + Three.js

当前版本具备比赛可演示的完整链路：登录鉴权、流式分析、会话管理、附件解析、结构化检索、多智能体协同、态势看板与情报查询。默认使用 siliconflow 远程 provider/embedding，同时保留本地模式作为增强能力。

---

## 1. 参赛定位

定位于“全国大学生计算机设计大赛 - 软件应用与开发 - Web 赛道”的安全运营原型系统，强调工程闭环而非单点算法。
...
- 流式聊天接口（SSE），支持 single 与 multi_agent
...
- 检索向量化默认使用 siliconflow，两种 embedding 模式（local / siliconflow）均可切换
- 模型路由支持：ollama、openai、deepseek、minimax、siliconflow
...
- 提供运行时配置 `/api/runtime-config` 与远程 embedding `/api/embeddings`
- 提供连通性测试 `/api/test_connection`（provider/search 两类测试）
...
- 消息展示：Markdown 渲染、代码块一键复制、多智能体过程面板与停止生成
...
- 聊天流启动先发送 `ping` 事件，保证前端快速进入流式消费
- 流式接口兼容 ASGI/WSGI：ASGI 下通过 `sync_to_async(next)` 驱动迭代，避免阻塞事件循环
- `mode=multi_agent` 支持 `agent_configs`，可对 rag/web/synthesis 三路分别指定 provider/model/key
- 会话历史使用文本协议保存，但做了协议转义与反转义，防止“用户：/回复：”破坏解析
- 多智能体元信息会序列化为 `【MULTI_AGENT_META】...` 块并持久化，历史恢复时可还原 Agent 状态
- 对“重新生成”做后端判定：识别“最后一轮 user+assistant 与当前输入一致”并回退上下文
- `upload_file` 针对 docx/xlsx 增加 Office ZIP 结构校验（条目数上限、解压后体积上限）
- OpenAI-compatible 调用失败时输出结构化错误：`provider/model/status_code/error_code/message/request_id`
- 本地 Ollama 不可用时自动降级到 siliconflow，并通过 `notice(scope=llm_fallback)` 告知前端
- `test_connection` 提供 provider/search 双链路探测，返回 `latency_ms`
- `runtime-config` 可回填服务端默认 API Key（各 provider + web search）
...
- 排序融合 `risk_level/source_priority/confidence/exact_match_boost`
- 结果聚合优先级：`cve_id -> ioc_value -> raw_content_hash -> tags -> db_type/source`
- “松弛召回”二次检索路径：主路径低分或空结果时自动降低阈值重试
- 远程 embedding 模式复用同一套候选融合与重排策略，保证结果口径一致

### 2.7 代码级前端特色功能（交互与体验）

- 路由守卫不仅做登录校验，还会在已登录状态下自动尝试同步 `runtime-config`
- 聊天流处理具备“空闲超时 + 缓冲区上限 + 可重试”机制，避免 SSE 长连接僵死
- SSE 事件按类型路由：`agent_chunk/agent_status/notice/content/think/metadata/error`
- 流式渲染采用“队列 + requestAnimationFrame + 定时窗口”批量刷帧，减少高频 chunk 导致的抖动与重排
- 聊天支持 `AbortController` 停止生成，前后端均可快速收敛状态
- 仅允许编辑最近一条用户消息，避免中间历史被破坏
- Markdown 渲染使用 `marked + highlight.js + DOMPurify`，并给代码块注入一键复制按钮
- 多智能体消息支持折叠面板，分别展示 rag/web 状态、内容、错误详情
- 会话草稿按会话维度持久化，带防抖写入，减少本地存储抖动
- 会话重命名联动导出目标会话 ID 与分析跳转历史，避免“重命名后失联”
- 看板图表具备全屏降级方案：Fullscreen API 不可用时回落到 Modal
- 图表下钻引导具备“首次提示记忆”能力（localStorage 标记已读）
- IntelQuery 请求支持 AbortController 取消旧请求，避免慢响应覆盖新筛选结果
- Intel 详情支持沉浸式阅读器、字段级复制反馈、关键词高亮缓存

### 2.8 代码级工程与运维特色功能

- 查询服务基于“文件签名 + 30s 刷新窗口 + RLock”实现缓存
- 支持查询缓存预热（启动后主动触发一次轻量查询）
- 导出文件名做白名单字符清洗，避免非法文件名与注入风险
- 导出支持 `all/current_page`、字段选择、`include_details`、CSV/JSON 双格式
- 前端全局错误通过 `deepsoc:api-error` 事件分发，401 通过 `deepsoc:unauthorized` 统一处理
- Vite 构建配置 manualChunks（`vue-vendor/naive/charts/three`）优化首屏加载
...
说明：代码层保留多 Provider 路由；比赛部署口径为仅使用 SiliconFlow 的 DeepSeek-V3.2，其它厂商选项作为占位与兼容入口保留。
...
- 后端可将上下文解析为结构化 history，前端恢复时会保留多行内容并还原多智能体元信息
...
- JSON/JSONL 结构化处理：
  ...
  - `_id/db_type/risk_level/cve_id/ioc_value/source/confidence/raw_content_hash/mitre_attack_id/tags` 写入 metadata
  - 自动补充 `record_file`、`record_line`
- 检索结果聚合优先级：`cve_id -> ioc_value -> raw_content_hash -> tags -> db_type/source`
- 模型配置支持 provider/model 与 embedding_mode/embedding_model 组合传参与自动回填

### 4.4 多智能体编排

- 并发阶段：VectorAgent（rag，内部证据）+ SearchAgent（web，外部情报）
- 汇总阶段：SynthesisAgent 在 rag/web 均完成后启动
- 编排器对 rag/web 输出尝试抽取 JSON，失败时生成降级 payload 进入 synthesis

### 4.5 模型与联网检索

- Provider 路由：ollama、openai、deepseek、minimax、siliconflow
- 比赛口径：仅使用 SiliconFlow deepseek-ai/DeepSeek-V3.2（前端显示名 DeepSeek-V3.2）
...
- composables：聊天会话流、设置、看板数据、全屏控制、情报查询逻辑
...
- Settings：
  ...
  - 页面会优先使用 `/api/runtime-config` 回填服务器侧默认 Key
...
- 同时可能返回 think、metadata 与 error 事件，其中 think 用于推理过程，metadata 用于耗时信息，error 事件包含 message 与兼容字段 chunk，可能附带 error_detail
- 会话流尾统一返回 done 事件
...
说明：如果压缩包里已包含完整 Python 环境，运行时优先使用该环境里的 python 和 pip；不要混用系统 Python。
...
如果希望本地向量检索链路完整可用，先安装 Ollama（Linux 官方脚本）：
...
说明：比赛口径主模型为 SiliconFlow 的 DeepSeek-V3.2；Ollama 是本地增强选项。
...
8. 上传链路具备安全设计：docx/xlsx 增加 ZIP 结构校验、条目上限、解压体积上限，体现安全工程意识
